...
% Server-89 batch: subjects 12,14,16,18 only (paired with _s92.m running the other 3
% subjects on server 92, to halve wall-clock time). Once both finish and their
% per-half .mat files have synced via Dropbox, re-run the unmodified
% run_splithalf_within_reglin_mean_12n.m (all 7 subjects) to merge: it will find
% every half already cached and just recompute the summary r_flat/r_iter2 arrays.
%
...
fprintf('\n=== Split-half within-subject (12n RegLin-Mean), server-89 subset ===\n');
...
fprintf('Done with server-89 subset. Per-half .mat files saved to %s\n', data_dir);
fprintf('Run the unmodified run_splithalf_within_reglin_mean_12n.m (all 7 subjects)\n');
fprintf('after both server batches finish, to produce the merged summary .mat.\n');
