%% run_sensitivity_s16_alpha_b025.m
% Prior mean perturbation, alpha=0.25, N_mc=10.
% Run on server 89 in parallel with run_sensitivity_s16_alpha_b050.m.
% rng seeds: 3001-3010 (ai_global=3, consistent with alpha_a ai=1,2).
...
data_dir = './zebra/';
out_mat  = fullfile(data_dir,'sensitivity_12n_s16_alpha_b025.mat');

n  = 12; N_mc = 10; alp = 0.25; ai_global = 3;
TR = 0.5;
...
fprintf('\n=== alpha=%.2f, N_mc=%d ===\n', alp, N_mc);
corr_mc = nan(1,N_mc); sign_mc = nan(1,N_mc); F_mc = nan(1,N_mc);

for mc = 1:N_mc
    rng(ai_global * 1000 + mc);
    ...
    try
        ...
        Aest = full(est.Ep.A); Fest = est.F;
    catch ME
        fprintf(' FAILED:%s', ME.message); Aest = nan(n,n); Fest = nan;
    end
    F_mc(mc) = Fest;
    if ~any(isnan(Aest(:)))
        corr_mc(mc) = corr(Aref(offdiag), Aest(offdiag));
        sign_mc(mc) = mean(sign(Aref(offdiag)) == sign(Aest(offdiag)));
    end
    fprintf('mc=%d: r=%.3f sign=%.1f%% (%.0fm)\n', mc, corr_mc(mc), sign_mc(mc)*100, toc(t0)/60);
end

fprintf('-> r=%.3f+/-%.3f  sign=%.1f+/-%.1f%%\n', ...
    nanmean(corr_mc), nanstd(corr_mc), nanmean(sign_mc)*100, nanstd(sign_mc)*100);
save(out_mat, 'alp','ai_global','N_mc','n','corr_mc','sign_mc','F_mc','-v7.3');
fprintf('Saved: %s\nDone.\n', out_mat);
