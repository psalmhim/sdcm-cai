function run_sc_binary_reglin_mean12n(subj_id)
% M2 binary-SC prior, RegLin obs, 12-node Mean signal (k-means, no z-score).
% Variance for connected pairs = 1/16; unconnected = 1/128.
% Prior mean (pE.A) kept at spm_dcm_calcium_priors default (not zeroed).
%
% Saves: zebra/subject_N_DCM_sc_binary_reglin_mean12n.mat
maxNumCompThreads(1);
...
DCM.name=sprintf('DCM_sc_binary_reglin_mean12n_s%d', subj_id);

fprintf('S%d sc-binary RegLin-Mean 12n... ', subj_id); rng(0); t0=tic;
DCM_est = spm_dcm_calcium_csd(DCM);
out = fullfile(data_dir, sprintf('subject_%d_DCM_sc_binary_reglin_mean12n.mat', subj_id));
