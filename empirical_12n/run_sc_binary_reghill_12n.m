function run_sc_binary_reghill_12n(subj_id)
% M2 binary SC prior, regional Hill obs (per-node alpha, global Kd/n), no demean.
maxNumCompThreads(1);
...
% Keep default weak positive A prior; only set SC-informed variance
...
% Regional observation: per-node alpha_i
pE.alpha  = zeros(n,1);
...
DCM.name=sprintf('DCM_sc_binary_reghill_12n_s%d', subj_id);

fprintf('S%d sc-binary regional-Hill 12n... ', subj_id); rng(0); t0=tic;
DCM_est = spm_dcm_calcium_csd(DCM);
out = fullfile(data_dir, sprintf('subject_%d_DCM_sc_binary_reghill_12n.mat', subj_id));
